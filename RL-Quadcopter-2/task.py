...
class Task():
    ...
    def __init__(self, init_pose=None, init_velocities=None, 
        ...
        #self.target_angular_v = target_angular_v if target_angular_v is not None else np.array([0., 180., 0.]) 
    ...
    def get_reward(self):
        ...
        bonus = 0.0
        for i in range(3):
            if (self.sim.pose[i] - self.target_pos[i]) < 0.1:
                bonus += 10
        reward = (1.0 - ( abs(self.sim.pose[:3] - self.target_pos ) ).sum() ) + bonus
